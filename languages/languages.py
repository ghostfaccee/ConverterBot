...
MESSAGES = {
    'ru' : {
        ...
        'help' : '📋 <b>Помощь:</b>\n <b>1.</b> <i>Чтобы конвертировать валюту одной страны в валюту другой страны используй команду /convert (число-1) (валюта) (валюта, в которую нужно перевести), где число-1 указывает на количество первой валюты. Сами валюты в обязательном порядке нужно указать в буквах, например USD, для доллара, RUB для рублей и так далее. Чтобы подробнее узнать о поддерживаемых валютах, советуем вам перейти по ссылке в оффициальной документации RatesFlow:</i> |link|. \n <b>2.</b> <i>Чтобы сменить язык пользователя, используйте команду /language.</i> \n <b>3.</b> <i>Чтобы просмотреть курс валют, информацию о пользователе или помощь, используйте команду /start и функции, которые она вам предоставит.</i> \n <b>4.</b> <i>Если все же возникли вопросы по поводу использования телеграм-бота RatesFlow, рекомендуем обратиться вам к оффициальной документации проекта:</i> |link|.',
